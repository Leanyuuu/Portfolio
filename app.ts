...
const motionIsAvailable = typeof window !== 'undefined' && (window as any).motion;
...
const mountSkills = () => {
  ...
  skills.forEach((skill, index) => {
    ...
    if (motionIsAvailable) {
      const { animate, stagger, spring } = (window as any).motion;
      const bar = card.querySelector('[data-skill-bar]') as HTMLElement;
      animate(card, { opacity: [0, 1], y: [12, 0] }, { delay: 0.05 * index, duration: 0.6, easing: 'ease-out' });
      animate(
        bar,
        { width: ['0%', `${skill.level}%`] },
        { delay: 0.3 + 0.05 * index, duration: 0.8, easing: spring({ stiffness: 200, damping: 20 }) }
      );
    }
  });
};
...
const mountProjects = () => {
  ...
  projects.forEach((project, index) => {
    ...
    card.className =
      'glass rounded-3xl border border-white/10 shadow-ios-card hover:-translate-y-1 transition-transform duration-300 overflow-hidden flex flex-col';

    const stack = project.stack
      .map((item) => `<span class="px-3 py-1 rounded-full text-xs bg-white/10 border border-white/15">${item}</span>`)
      .join('');
